	...
	<script type="text/javascript">
		...
		function build_emps_table(result) {
			...
			$.each(emps, function(index, item) {
				...
				var editBtn = $("<shiro:hasRole name="admin"><button></button></shiro:hasRole>").addClass(
						"btn btn-primary btn-sm edit_btn").append(
						$("<spand></spand>").addClass(
								"glyphicon glyphicon-pencil")).append("编辑");
				//为编辑按钮添加一个自定义属性，表示员工id
				editBtn.attr("edit_id", item.empId);
				var delBtn = $("<shiro:hasRole name="admin"><button></button></shiro:hasRole>").addClass(
	    ...
	    if ('WebSocket' in window) {
	        websocket = new WebSocket("ws://localhost:8081/ssm-crud/ws/bitcoinServer");
