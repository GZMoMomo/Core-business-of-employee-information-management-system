...
public class EmployeeController {
	...
	/**
	 * AJAX发送put请求有可能会拿不到数据 Tomcat识别到put不会封装请求体中的数据为map,只有post才封装请求体
	 * 配置HttpPutFormContentFilter 将请求体中的数据解析包装成一个map
	 * request被重新包装，getparameter被重写，从自己封装的map取数据 员工更新
	 */
	@RequiresRoles("admin")
	@ResponseBody
	...
	/**
	 * 员工保存 支持JSR303校验 hibernate validator
	 * 
	 * @return
	 */
	@RequiresRoles("admin")
	...
	public String loginn(org.springframework.ui.Model model,@RequestParam(value="name")String name,@RequestParam(value="password")String password) {
